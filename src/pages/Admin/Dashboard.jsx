    ...
    <DashboardLayout activeMenu="Dashboard">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          ...
          <button onClick={() => navigate("/admin/tasks")} className="flex items-center gap-1 text-sm text-[#1368EC] hover:underline font-medium">
            ...
            <tbody>
              {data?.recentTasks?.map((task, i) => (
                <tr key={i} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-3.5">
                    <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${getPriorityColor(task.priority)}`}>{task.priority}</span>
                  </td>
