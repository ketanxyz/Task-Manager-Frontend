    ...
    <DashboardLayout activeMenu="My Tasks">
        ...
        <div className="flex gap-1 bg-slate-100 rounded-xl p-1">
          {TABS.map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              ...
              {tab} <span className={activeTab === tab ? "text-[#1368EC]" : "text-slate-400"}>{tabCount(tab)}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(task => (
            <TaskCard key={task._id} task={task} onClick={() => navigate(`/user/tasks-details/${task._id}`)} />
          ))}
        </div>
      )}
    </DashboardLayout>
